...
const SignupForm = () => {
  ...
  const addUser = async (userEmail,userId) => {
    try {

        // console.log("Create new User entry");
        const newUser = {
          chatHistoryBlobPath: "/chat/" + userEmail,
          customerId: null,
          email: userEmail,
          userId: userId,
          paymentHistoryBlobPath: "/payment/" + userEmail,
          token: 10000
        };

        await setDoc(doc(db, "userInfo", userId), newUser);
  ...
  const checkUserByUserId = async (userId) => {
    try {
  ...
  const fetchUserData = async (user) => {
    ...
    const exists = await checkUserByUserId(userId);
    if (!exists) {
      await addUser(userEmail, userId);
    }
